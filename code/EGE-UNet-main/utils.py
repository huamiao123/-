...
class myRandomScaleCrop:
    """Random zoom (scale_range) then random crop back to original size.

    Amplifies small-lesion training signal (zoom-in) and exposes
    scale variation (zoom-out). Mask uses nearest interpolation;
    padding uses reflection for image / zeros (background) for mask.
    """
    def __init__(self, p=0.5, scale_range=(0.5, 2.0)):
        self.p = p
    ...
    def __call__(self, data):
        ...
        nH, nW = max(int(H * s), 16), max(int(W * s), 16)
        image = TF.resize(image, [nH, nW], antialias=True)
        mask = TF.resize(mask, [nH, nW],
                         interpolation=torchvision_t.InterpolationMode.NEAREST)
        padH, padW = max(H - nH, 0), max(W - nW, 0)
        if padH > 0 or padW > 0:
            image = TF.pad(image, [padW // 2, padH // 2, padW - padW // 2, padH - padH // 2],
                          padding_mode='reflect')
            mask = TF.pad(mask, [padW // 2, padH // 2, padW - padW // 2, padH - padH // 2],
                          padding_mode='constant', fill=0)
        Hn, Wn = image.shape[-2], image.shape[-1]
        top = random.randint(0, Hn - H)
        left = random.randint(0, Wn - W)
        return image[..., top:top + H, left:left + W], mask[..., top:top + H, left:left + W]
